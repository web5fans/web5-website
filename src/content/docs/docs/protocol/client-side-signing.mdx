---
title: Client-Side Signing and Key Migration
---
...
This page outlines methods for performing cryptographic signing operations directly on the client side rather than delegating this responsibility to the Personal Data Server (PDS).
It specifies two approaches for managing signing keys on the client: (1) deterministic key generation using RFC6979, and (2) random key generation with import/export capabilities for multi-device usage.
...
A formal specification for client-side signing can be found in the [wip-05](https://github.com/web5fans/web5-wips/blob/master/05.md).
