---
title: did:ckb
description: Understanding the did:ckb method for decentralized identities on Nervos CKB
---

The `did:ckb` method is a Decentralized Identifier (DID) specification that leverages the Nervos CKB blockchain to create, manage, and resolve DIDs. This protocol provides a decentralized way to establish digital identities with strong security guarantees through blockchain technology.
...
The protocol achieves several key goals:
...
3. A unique identifier is derived from transaction details using cryptographic hashing
...
If multiple cells exist for the same DID (due to conflicts), the protocol uses rules to determine which one is authoritative - typically the earliest created cell.
...
The `did:ckb` protocol provides robust security through:

- **Blockchain Consensus**: CKB's Byzantine fault-tolerant consensus prevents unauthorized modifications
- **Cryptographic Verification**: All operations use proven cryptographic primitives
- **Authorization Scripts**: Flexible ownership models support various authentication schemes
- **Immutability**: Once created, transaction history cannot be altered

### Protecting Against Attacks

- **Double-spending**: Prevented by CKB's UTXO model and consensus
...
`did:ckb` is designed to work seamlessly with the broader Web5 ecosystem, providing a blockchain-based identity layer that complements Decentralized Web Nodes (DWNs) and other Web5 components.
