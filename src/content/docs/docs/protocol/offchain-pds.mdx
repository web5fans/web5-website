---
title: did:ckb PDS
description: Implementation guidance for did:ckb Personal Data Server (PDS) integration with CKB
---

PDS (Personal Data Server) is a crucial component in AT Protocol, responsible for managing user data storage.
This page provides a implementation guidance for the `did:ckb` PDS system, which extends the AT Protocol infrastructure to support the `did:ckb` DID method through integration with the CKB blockchain.

A formal specification for the `did:ckb` PDS can be found in the [wip-03](https://github.com/web5fans/web5-wips/blob/master/03.md).
...
![did:ckb PDS](/images/docs/protocol/did-manage-two-steps.png)
